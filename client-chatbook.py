...
class FirstScreen(tk.Tk):
    def __init__(self):
        ...
        background = Image.open("images/she.jpg")
        ...
        submit_button = tk.Button(self.first_frame, text="Connect", font="lucida 12 bold", padx=30, cursor="hand2",
                                  command=self.process_and_save, bg="#16cade", relief="solid", bd=2)
        submit_button.place(x=200, y=275)
        submit_button.place(x=200, y=325)
    ...
    def process_data(self):
        if self.username_entry.get():
            ...
            try:
                client_socket.connect(("192.168.1.76", 8080))
                status = client_socket.recv(1024).decode('utf-16')
                ...
                return
            client_socket.send(self.user.encode('utf-16'))
            ...
            if client_socket.recv(1024).decode('utf-16') == 'received':
                client_socket.send(str(self.image_extension).strip().encode('utf-16'))
            ...
            client_socket.send('image_received'.encode('utf-16'))
...
class ChatScreen(tk.Canvas):
    def __init__(self, parent, first_frame, client_socket, clients_connected, user_id):
        ...
        y_co = int((screen_height / 2) - (730 / 2))
        ...
        tk.Label(self, text="Chat book", font="lucida 15 bold", padx=20, fg="green",
        ...
        clear_button = tk.Button(self, text="Clear Chat", fg="#f44242", font="lucida 11 bold", bg="#7d7d7d", padx=10,
                                 relief="solid", bd=2, command=self.clear_chat)
        clear_button.place(x=500, y=680)
    ...
    def clear_chat(self):
        ...
        self.scrollable_frame = tk.Frame(self.canvas, bg="#595656")
        # ... create and pack messages in the scrollable_frame ..
    ...
    def receive_data(self):
        while True:
            try:
                data_type = self.client_socket.recv(1024).decode('utf-16')
